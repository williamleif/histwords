...
import numpy as np
from cooccurrence.matstore import reindex
...
class Explicit:
    ...
    @classmethod
    def load(cls, path, normalize=True, restricted_context=None, thresh=None, **kwargs):
    ...
    def reindex(self, word_list, normalize=False, restrict_context=True):
        new_embed = self.get_subembed(word_list, restrict_context=restrict_context)
        w_set = set(new_embed.iw)
        w_map = {new_embed.wi[word]:i for i, word in enumerate(word_list) if word in w_set}
        if restrict_context:
            c_set = set(new_embed.ic)
            c_map = {new_embed.ci[word]:i for i, word in enumerate(word_list) if word in c_set}
            c_word_list = word_list
            max_c = len(word_list) - 1
        else:
            c_map = {i:i for i in range(len(self.ic))}
            c_word_list = self.ic
            max_c = len(self.ic) - 1
        mat = reindex(new_embed.m, w_map, c_map, len(word_list) - 1, max_c)
        return Explicit(mat, word_list, c_word_list, normalize=normalize)
